...
clean_dir_key=$(func_parser_key "${lines[30]}")
clean_img_dir=$(func_parser_value "${lines[30]}")
noisy_dir_key=$(func_parser_key "${lines[31]}")
noisy_img_dir=$(func_parser_value "${lines[31]}")
benchmark_key=$(func_parser_key "${lines[32]}")
benchmark_value=$(func_parser_value "${lines[32]}")
    ...
    for use_gpu in ${use_gpu_list[*]}; do
        ...
        if [ ${use_gpu} = "False" ] || [ ${use_gpu} = "cpu" ]; then
            for batch_size in ${batch_size_list[*]}; do
                ...
                set_clean_data=$(func_set_params "${clean_dir_key}" "${_clean_dir}")
                set_noisy_data=$(func_set_params "${noisy_dir_key}" "${_noisy_dir}")
                set_benchmark=$(func_set_params "${benchmark_key}" "${benchmark_value}")
                set_batchsize=$(func_set_params "${batch_size_key}" "${batch_size}")
                set_model_dir=$(func_set_params "${infer_model_key}" "${_model_dir}")
                command="${_python} ${_script} ${use_gpu_key}=${use_gpu} ${set_model_dir} ${set_batchsize} ${set_clean_data} ${set_noisy_data} ${set_benchmark} > ${_save_log_path} 2>&1 "
                eval $command
    ...
    for gpu in ${gpu_list[*]}; do
        ...
        for trainer in ${trainer_list[*]}; do
            ...
            if [ ${run_export} != "null" ]; then
                ...
                export_cmd="${python} ${run_export} ${set_save_infer_key}"
                eval $export_cmd
                ...
                func_inference "${python}" "${inference_py}" "${infer_model_dir}" "${LOG_PATH}" "${clean_img_dir}" "${noisy_img_dir}"
